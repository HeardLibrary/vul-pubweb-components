      ...
      fetch(resource, options).then(function (response) {
        ...
        navigation[division].forEach(function (item) {
          if (item.hasOwnProperty('dropdown')) {
            ...
            html += "<button type=\"button\" id=\"".concat(item.text.toLowerCase(), "Btn\" data-toggle=\"dropdown\" aria-expanded=\"false\" data-display=\"static\" class=\"vumainmenu__dropdown dropdown-toggle btn\">");
            html += "".concat(item.text, " <span class=\"vumainmenu__text--gold fas fa-caret-down\" aria-hidden=\"true\"></span>");
            html += "</button>";
            html += "<div class=\"dropdown-menu\" aria-labelledby=\"".concat(item.text.toLowerCase(), "Btn\">");
            item.dropdown.forEach(function (link) {
              html += "<a class=\"dropdown-item\" href=\"".concat(link.url, "\">").concat(link.text, "</a>");
            });
            html += "</div>";
            html += "</li>";
          } else {
